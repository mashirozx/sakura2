...
class Sakura_Walker_Drawer_Nav_Menu extends Walker_Nav_Menu
{

    /**
     * Starts the list before the elements are added.
     *
     * Note: here <ul> display only for sub items.
     *
     * @since 3.0.0
     *
    ...
    public function start_el(&$output, $item, $depth = 0, $args = null, $id = 0)
    {
        ...
        if (isset($matches[1])) {
            ...
            preg_match('/\{(.*?)\}(.*)/i', $title, $match_iconfont, PREG_UNMATCHED_AS_NULL);
            if (isset($match_iconfont[1])) {
                $icon = $match_iconfont[1];
                $label = $match_iconfont[2];
                $flag = 2;
            } else {
                $label = $title;
                $flag = 3;
